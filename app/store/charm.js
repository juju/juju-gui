...
var CharmStore = Y.Base.create('charm', Y.Base, [], {
  ...
  // We might want a get_charm or getById in the future.
    ...
    this.get('datasource').sendRequest({
      ...
      callback: {
        success: function(io_request) {
          options.success(Y.JSON.parse(io_request.response.results[0].responseText));
        },
    ...
    this.get('datasource').sendRequest({
      ...
      callback: {
        'success': Y.bind(function(io_request) {
          ...
          options.success(
            this._normalizeCharms(result_set.results, options.defaultSeries));
        }, this),
        'failure': options.failure
      }});
  },
