...
describe('GitHub SSH key fetching', () => {
  ...
  it('fetches github SSH keys', () => {
    githubSSHKeys({sendGetRequest: sendGetRequest}, 'rose', callback);
    const args = sendGetRequest.args[0];
    // Make sure the correct URL was called.
    assert.equal(args[0], 'https://api.github.com/users/rose/keys');
  ...
  it('surfaces errors', () => {
    githubSSHKeys({sendGetRequest: sendGetRequest}, 'bad-wolf', callback);
    const args = sendGetRequest.args[0];
    // Ensure our URL is the bad one
    assert.equal(args[0], 'https://api.github.com/users/bad-wolf/keys');
    assert.equal(typeof args[6], 'function');
