...
var viewsUtils = window.viewsUtils;
  ...
  getNearestConnector: {
    value: function(box_or_xy) {
      ...
      Object.keys(connectors).forEach(key => {
        ...
        if (!viewsUtils.isValue(result) || d < shortest_d) {
          shortest_d = d;
          result = ep;
        }
      }, this);
...
environmentUtils.toBoundingBoxes = function(module, services, existing, env) {
  ...
  Object.keys(result).forEach(key => {
    if (!viewsUtils.isValue(services.getById(key))) {
      delete result[key];
    }
  });
  services.each(
    function(service) {
      ...
      if (!service.get('icon') && service.get('charm')) {
        ...
        icon = viewsUtils.getIconPath(charmId, null, env);
