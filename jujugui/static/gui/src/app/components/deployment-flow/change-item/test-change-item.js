...
describe('DeploymentChangeItem', function() {
  ...
  const renderComponent = (options = {}) =>
    enzyme.shallow(
      <DeploymentChangeItem change={options.change || change} showTime={options.showTime} />
    );
    ...
    const expected = (
      <div className="deployment-change-item">
        <span className="deployment-change-item__change">
          <SvgIcon className="deployment-change-item__icon" name="my-icon" size="16" />
          Django was added
          <span className="deployment-change-item__change-command">juju deploy cs:django</span>
        </span>
        <span className="deployment-change-item__time">{change.time}</span>
      </div>
    );
  ...
  it('can display an svg icon', function() {
    ...
    const expected = <img alt="" className="deployment-change-item__icon" src="my-icon.svg" />;
