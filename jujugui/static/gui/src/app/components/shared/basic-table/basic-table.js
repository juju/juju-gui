...
class BasicTable extends React.Component {
  ...
  */
  ...
  render() {
    const classes = classNames('basic-table', this.props.tableClasses);
    return (
      <table className={classes} role="grid">
        <thead>{this._generateRow(true, {columns: this.props.headers})}</thead>
        <tbody>{this._generateContent()}</tbody>
      </table>
    );
  }
}
