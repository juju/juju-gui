...
describe('NotificationListItem', function() {
  ...
  it('renders a notification list item', () => {
    ...
    var classes =
      'notification-list-item notification-list-item--info ' +
  ...
  it('updates class and calls to remove itself after hiding', () => {
    ...
    assert.isTrue(wrapper.prop('className').includes('notification-list-item--visible'));
    assert.isFalse(wrapper.prop('className').includes('notification-list-item--hidden'));
    wrapper.find('.notification-list-item__hide').simulate('click');
    wrapper.update();
    // After a click it should have the classes updated.
    assert.isFalse(wrapper.prop('className').includes('notification-list-item--visible'));
    assert.isTrue(wrapper.prop('className').includes('notification-list-item--hidden'));
  });
