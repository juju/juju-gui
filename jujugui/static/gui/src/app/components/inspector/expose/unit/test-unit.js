...
describe('InspectorExposeUnit', function() {
  const renderComponent = (options = {}) =>
    enzyme.shallow(
      <InspectorExposeUnit action={options.action || sinon.stub()} unit={options.unit} />
    );
  ...
  it('can render the unit', function() {
    ...
    var expected = (
      <li
        ...
        tabIndex="0"
      >
        <div className="inspector-expose__unit-detail">django/1</div>
        <ul className="inspector-expose__unit-list">
          <li className="inspector-expose__item" key="20.20.20.199:9000-10000/udp">
            <span>{'20.20.20.199:9000-10000/udp'}</span>
          </li>
          <li className="inspector-expose__item" key="https://20.20.20.199:443">
            <a
          ...
          <li className="inspector-expose__item" key="http://20.20.20.199:8080">
            <a
  ...
  it('can render the unit without a public address', function() {
    ...
    assert.equal(
      wrapper
        .find('.inspector-expose__unit-detail')
        .at(1)
        .text(),
      'No public address'
    );
