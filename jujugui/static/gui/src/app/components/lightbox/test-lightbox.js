...
describe('Lightbox', function() {
  ...
  it('renders', () => {
    ...
    const expected = (
      <div className="lightbox" onClick={wrapper.prop('onClick')}>
        ...
        >
          <div className="lightbox__content">Hi</div>
          <div className="lightbox__caption">Test caption</div>
        </div>
      </div>
    );
  ...
  describe('slides', () => {
    ...
    beforeEach(() => {
      slideList = [<p key="1">Slide 1</p>, <p key="2">Slide 2</p>, <p key="3">Slide 3</p>];
    });
          ...
          <div className="lightbox__navigation">
            ...
            <ul className="lightbox__navigation-bullets">
              <li
                ...
                onClick={wrapper
                  .find('.lightbox__navigation-bullet')
                  .at(0)
                  .prop('onClick')}
              >
                &bull;
    ...
    it('moves to the next slide when _nextSlide is called', () => {
      ...
      wrapper.update();
      assert.equal(wrapper.find('.lightbox__navigation-previous').prop('disabled'), false);
    ...
    it('disables the next slide button when the last slide is active', () => {
      ...
      wrapper.update();
      assert.equal(wrapper.find('.lightbox__navigation-previous').prop('disabled'), false);
      assert.equal(wrapper.find('.lightbox__navigation-next').prop('disabled'), true);
    });
    ...
    it('does not move past the first slide', () => {
      ...
      instance._goToSlide(-1);
      wrapper.update();
      assert.equal(wrapper.find('.lightbox__navigation-previous').prop('disabled'), true);
      const bullets = wrapper.find('.lightbox__navigation-bullet');
      assert.equal(
        bullets
          .at(0)
