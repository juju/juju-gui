...
describe('Terminal', () => {
      ...
      <div className="juju-shell">
        <div className="juju-shell__header">
          ...
          <div className="juju-shell__header-actions">
            <span onClick={actions.at(0).prop('onClick')} role="button" tabIndex="0">
              <SvgIcon name="minimize-bar_16" size="16" />
            </span>
            <span onClick={actions.at(1).prop('onClick')} role="button" tabIndex="0">
              <SvgIcon name="maximize-bar_16" size="16" />
            </span>
            <span onClick={actions.at(2).prop('onClick')} role="button" tabIndex="0">
  ...
  it('displays a welcome message when the session is ready', () => {
    checkWelcomeMessage(
      ...
      [['connected to workspace'], ['these are'], ['the voyages']]
    );
  });
  ...
  it('displays a welcome message when the session is ready (legacy)', () => {
    checkWelcomeMessage(
      ...
      [['connected to workspace'], ['session is ready']]
    );
  });
  ...
  it('can be closed by clicking the X', () => {
    ...
    wrapper
      .find('.juju-shell__header-actions span')
      .at(2)
      .simulate('click');
  ...
  it('can be resized by clicking the two resize buttons', () => {
    ...
    assert.equal(termElement.prop('className').includes('juju-shell__terminal--min'), false);
    ...
    assert.equal(termElement2.prop('className').includes('juju-shell__terminal--min'), true);
