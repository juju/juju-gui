...
describe('MachineViewColumn', function() {
  ...
  it('can render', function() {
    ...
    const expected = (
      <div className="machine-view__column">
        <MachineViewHeader
          acl={acl.reshape(MachineViewHeader.DecoratedComponent.propTypes.acl)}
          activeMenuItem="name"
          droppable={true}
          dropUnit={sinon.stub()}
          menuItems={[]}
  ...
  it('can render in droppable mode', function() {
    ...
    assert.equal(wrapper.prop('className').includes('machine-view__column--droppable'), true);
  });

  it('can render in drop mode', function() {
    const wrapper = renderComponent({isOver: true});
    assert.equal(wrapper.prop('className').includes('machine-view__column--drop'), true);
  });
});
