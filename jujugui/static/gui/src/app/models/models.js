...
window.yui.add(
  ...
  */
    ...
    */
        updateConfig: function(changeConfig) {
          ...
          this.set('environmentConfig', Y.merge(this.get('environmentConfig'), changeConfig));
    ...
    */
      getServicesFromCharmName: function(charmName) {
        return this.filter(function(service) {
          ...
          if (charm) {
            // In tests charm can be undefined
    ...
    */
      _generateServiceName: function(charmName, services, counter = 0) {
        ...
        var name = counter > 0 ? charmName + '-' + this._numToLetter(counter) : charmName;
        ...
        // counter.
        return match ? this._generateServiceName(charmName, services, (counter += 1)) : name;
      },
   ...
   */
    ...
    */
    ...
    @class RemoteService
  */
    var RemoteService = Y.Base.create(
      'remoteService',
      Y.Model,
      [],
      {
  ...
  */
    var ServiceUnitList = Y.Base.create(
      'serviceUnitList',
      Y.LazyModelList,
      [],
      {
    ...
    */
        process_delta: function(action, data, db) {
          ...
          if (
            existingCharmUrl &&
            existingCharmUrl !== instance.charmUrl &&
    ...
    */
        add: function(models, allowed) {
          if (!allowed && this.get('preventDirectChanges')) {
            throw new Error(
              'direct calls to units.add() are not allowed: ' + 'use db.addUnits() instead'
            );
          }
    ...
    */
        _determineCategoryType: function(category) {
          if (/fail|error/.test(category)) {
            return 'error';
          }
          if (/landscape/.test(category)) {
            return 'landscape';
          }
          if (category === 'running') {
            return 'running';
          }
          if (category === 'uncommitted') {
            return 'uncommitted';
          }
          return 'pending';
        },
    ...
    */
     ...
     */
        update_service_unit_aggregates: function(service) {
          ...
          var sum = Object.keys(aggregate[0])
            .map(k => aggregate[0][k])
            .reduce((a, b) => {
              return a + b;
            }, 0);
    ...
    */
    ...
    var RelationList = Y.Base.create(
      'relationList',
      Y.ModelList,
      [],
      {
    ...
    */
        getRelationFromEndpoints: function(endpoints) {
          ...
          this.some(function(relation) {
            matching = this.compareRelationEndpoints(relation.get('endpoints'), endpoints);
            if (matching) {
    ...
    */
        _compareEndpointSets: function(endpointSetA, endpointSetB) {
          ...
          for (var i = 0, l = endpointSetA.length; i < l; i += 1) {
            ...
            // name should match.
            if (endpointA[0] !== endpointB[0] || endpointA[1].name !== endpointB[1].name) {
              return false;
            }
          }
          return true;
    ...
    var Notification = Y.Base.create(
      'notification',
      Y.Model,
      [],
      {},
    ...
    var NotificationList = Y.Base.create(
      'notificationList',
      Y.ModelList,
      [],
      {
