...
class EnvironmentView {
  ...
  render() {
    ...
    if (!this._rendered) {
      ReactDOM.render(<Environment />, container);
      this._rendered = true;
    }
    ...
    container
      .querySelector('.environment-help .plus-service')
      .addEventListener('click', this.boundPlusClickHandler);
  ...
  destructor() {
    if (this.boundRenderedHandler) {
      document.removeEventListener('topo.rendered', this.boundRenderedHandler);
    }
    if (this.boundPlusClickHandler) {
      const container = this.getContainer();
      container
        .querySelector('.environment-help .plus-service')
        .removeEventListener('click', this.boundPlusClickHandler);
    }
    if (this.topo) {
  ...
  */
  fadeHelpIndicator(fade) {
    ...
    const dropMessage = container.querySelector('.environment-help__drop-message');
    ...
    if (helpImg) {
      if (fade) {
        if (existingApps) {
          ...
          container.querySelector('.environment-help').classList.remove('shrink');
        }
  ...
  */
  _handlePlusClick(e) {
    this.sendAnalytics('Onboarding', 'Click', 'Onboarding plus');
