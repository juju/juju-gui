...
describe('EnvironmentView', function() {
  describe('juju environment view', function() {
    ...
    beforeEach(function() {
      ...
      var charmData = testUtils.loadFixture('data/mysql-api-response.json', true);
    ...
    it('must handle the window resize event', function(done) {
      ...
      const beforeHandler = e => {
        document.removeEventListener('beforePageSizeRecalculation', beforeHandler);
        // This event must be fired.
        beforeResizeEventFired = true;
      };
      const afterHandler = e => {
        document.removeEventListener('afterPageSizeRecalculation', afterHandler);
        // This event must be fired.
    ...
    it('must render services blocks correctly', function() {
      ...
      view.render();
      const serviceBlock = container.querySelector('.service').querySelector('.service-block');
    ...
    it('properly renders the create relation icon', function() {
      ...
      assert.equal(circles.item(1).classList.contains('relation-button__link'), true);
    ...
    it('properly renders the create relation icon using staticURL', function() {
      ...
      view.render();
      const relationIcon = container
        .querySelector('.service')
        .querySelector('.relation-button');
    ...
    it('must be able to render service blocks and relations', function() {
      ...
      container.querySelectorAll('.rel-group .relation.subordinate').length.should.equal(1);
    ...
    it('must be able to render subordinate and normal services', function(done) {
      // Create an instance of EnvironmentView with custom env
      var view = new EnvironmentView({
        container: container,
        db: db,
      ...
      view.render();
      container.querySelectorAll('.service').length.should.equal(4);
      container.querySelectorAll('.subordinate.service').length.should.equal(1);
    ...
    it('must be able to display service icons as pending deletion', function() {
      ...
      view.render();
      assert.equal(
        container.querySelector('.service .service-block').getAttribute('stroke'),
        '#19b6ee'
      );
    });
    ...
    it('must properly count subordinate relations', function() {
      ...
      function validateRelationCount(serviceNode, module, count) {
        ...
        return module.subordinateRelationsForService(service).length === count;
      }
      ...
      validateRelationCount(
        container.querySelector('.subordinate.service'),
        relationModule,
        2
      ).should.equal(true);
    });
    ...
    it('must resize the service health graph properly when units are added', function() {
      new EnvironmentView({
        container: container,
        db: db,
        env: env,
      ...
      function chartSizedProperly(serviceNode) {
        ...
        var maskWidth = node.select('.service-health-mask').attr('width');
    ...
    it('must recalculate relation endpoints when services are resized', function() {
      new EnvironmentView({
        container: container,
        db: db,
        env: env,
      ...
      function endpointsCalculatedProperly(relation) {
        ...
        var connectors = boxpair.source.getConnectorPair(boxpair.target);

        return (
          cmp(line.attr('x1'), connectors[0][0]) &&
      ...
      container.querySelectorAll('.rel-group').forEach(relationGroup => {
        endpointsCalculatedProperly(relationGroup).should.equal(true);
      });
    ...
    it('must be able to place new services properly', function() {
      ...
      container.querySelectorAll('.service').forEach(serviceNode => {
        ...
        properTransform.test(serviceNode.getAttribute('transform')).should.equal(true);
      });
    ...
    it('must be able to render subordinate relation indicators', function() {
      new EnvironmentView({
        container: container,
        db: db,
        env: env,
    ...
    it(
      'must be able to compute rect sizes based on the svg and' + ' viewport size',
      function() {
        var view = new EnvironmentView({
          container: container,
          db: db,
        ...
        parseInt(svg.querySelector('g').getAttribute('height'), 10).should.equal(
          parseInt(window.getComputedStyle(svg).getPropertyValue('height'), 10)
        );
        parseInt(svg.querySelector('g').getAttribute('width'), 10).should.equal(
    ...
    // Ensure that sizes are computed properly
    it('must be able to compute sizes by the viewport with a minimum', function() {
      ...
      parseInt(svg.getAttribute('height'), 10).should.be.above(199);
      // Destroy the navbar
      navbar.remove(true);
    ...
    it('must be able to add a relation from the service menu', function() {
      ...
      var endpoints = {},
        ...
        nextServiceName = serviceNode.nextSibling.getAttribute('data-name');
      ...
      container.querySelectorAll('.selectable-service').length.should.equal(2);
      container.querySelectorAll('.dragline').length.should.equal(1);
      ...
      container.querySelectorAll('.selectable-service').length.should.equal(0);
    ...
    it('must be able to remove a relation between services', function(done) {
      ...
      var relation = container.querySelector(
        '#' +
          relationUtils.generateSafeDOMId('mysql:db wordpress:db', getParentId(view)) +
          ' .rel-indicator'
      );
    ...
    it('builds a menu of relations in a collection', function() {
      ...
      assert.equal(
        menu.querySelector('.relation-container').getAttribute('data-relationid'),
      ...
      assert.equal(module.relationMenuRelation.id, 'mysql:db wordpress:db');

      // Multiple relations
      relation = container.querySelector(
        '#' +
          relationUtils.generateSafeDOMId('mysql:db mediawiki:db', getParentId(view)) +
          ' .rel-indicator'
      );
      relation.dispatchEvent(click);
      menu = container.querySelector('#relation-menu');
      ...
      assert.equal(menu.querySelectorAll('.relation-container.error').length, 1);
      assert.equal(menu.querySelectorAll('.relation-container.running').length, 1);
    });

    it('shows relation status with an indicator', function() {
      view.render();
      var reduceData = function() {
        return view.topo.vis
          .selectAll('.rel-indicator image')
          .data()
          .map(function(datum) {
      ...
      var reduceImages = function() {
        return view.topo.vis.selectAll('.rel-indicator image')[0].map(function(image) {
    ...
    it('must not remove a deployed subordinate relation between services', function() {
      view = new EnvironmentView({
        container: container,
        db: db,
        env: env,
    ...
    it('should remove a pending subordinate relation between services', function() {
      view = new EnvironmentView({
        container: container,
        db: db,
        env: env,
    ...
    it('should stop creating a relation if the background is clicked', function() {
      var db = new models.Database({
          getECS: sinon.stub().returns({changeSet: {}})
        }),
    ...
    it('stores relations in collections', function() {
      ...
      assert.equal(view.topo.vis.selectAll('.relation').size(), module.relations.length);
      assert.equal(module.relations.length, 3);
      assert.equal(
        db.relations.filter(function(relation) {
    ...
    describe('onboarding integration with the environment', function() {
      it('shows/hides the integrated button when a service is added', function() {
        db = new models.Database({
          getECS: sinon.stub().returns({changeSet: {}})
        });
        view.db = db;
  ...
  describe('view model support infrastructure', function() {
    ...
    it('must be able to get us nearest connectors when snapping to poles', function() {
      ...
      b1.getNearestConnector(b2).should.eql(b1.connectors.bottom);

      b2.getNearestConnector(b1).should.eql(b2.connectors.top);

      b1.getConnectorPair(b2).should.eql([b1.connectors.bottom, b2.connectors.top]);
    });

    it('must be able to get us nearest connectors when centering', function() {
      var b1 = environmentUtils.BoundingBox(module, service),
        b2 = environmentUtils.BoundingBox(module, service);
      ...
      b1.getNearestConnector(b2).should.eql(b1.connectors.center);

      b2.getNearestConnector(b1).should.eql(b2.connectors.center);
    });
    ...
    it('must be able to save and restore old position information', function() {
      var b1 = environmentUtils.BoundingBox(module, service),
        b2 = environmentUtils.BoundingBox(module, service);
    ...
    it('must be able to update position data and not touch model data', function() {
      var b1 = environmentUtils.BoundingBox(module, service);
      b1.x = 0;
      b1.y = 0;
      b1.w = 100;
    ...
    it('must be able to map from sequence of models to boundingboxes', function() {
      var services = new models.ServiceList();
      services.add([{id: 'mysql'}, {id: 'haproxy'}, {id: 'memcache'}, {id: 'wordpress'}]);
    ...
    it('must cull removed services from the existing list', function() {
      var services = new models.ServiceList();
      services.add([{id: 'mysql', exposed: false}, {id: 'memcache'}, {id: 'wordpress'}]);
      var existing = {
        mysql: {},
