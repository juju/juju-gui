...
const topologyUtils = require('./utils');
const Component = require('../../d3-components/d3-components');
...
class Topology extends Component {
  constructor(options={}) {
    ...
    */
    this.size = options.size || [640, 480];
    /*
  ...
  */
  servicePointOutside(includeVertices) {
    ...
    var vertices = topologyUtils.serviceBoxesToVertices(existingBoxes);
    if (includeVertices && includeVertices.length) {
      includeVertices.forEach(function(item) {
        vertices.push(item);
      });
    }
    return topologyUtils.pointOutside(vertices, this.servicePadding);
  }
