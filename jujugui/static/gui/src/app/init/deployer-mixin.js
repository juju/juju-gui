...
*/
const DeployerMixin = superclass =>
          ...
          (error, ids) => {
            if (error !== null) {
              db.notifications.add({
                ...
                message:
                  `Could not add requested resources for ${charmId}. ` +
    ...
    _addCharmCallbackHandler(charm, response) {
      ...
      if (response.err) {
        db.notifications.add({
          ...
          message: 'Could not add requested charm. Server responded with: ' + response.err,
          level: 'error'
        });
        return;
      }
    ...
    _addUnitCallback(ghostUnit, evt) {
      ...
      if (evt.err) {
        ...
        db.notifications.add({
          ...
          message: 'Could not add the requested unit. Server ' + 'responded with: ' + evt.err,
          level: 'error'
        });
        return;
      }
