...
YUI().use(['yui'], function(Y) {
  ...
  function rewriteRelativeURLs(contents, rootPath, outputFile) {
    var assetPaths = extractRelativePaths(contents);
    Y.Array.each(assetPaths, function(assetPath) {
      var absoluteAssetPath = path.resolve(outputFile, assetPath);
      var relativeAssetPath = path.relative(outputFile, rootPath);
      console.log(absoluteAssetPath);
      console.log(relativeAssetPath);
      console.log(rootPath);
//      contents = contents.replace(assetPath, servedAssetPath);
    });
    return contents;
  }

  // Combine one or more CSS files into a single compressed file.
  function combineCSS(files, rootPath, outputFile) {
    var chunks = [];
    Y.Array.each(files, function(inputFile) {
      var contents = fs.readFileSync(inputFile, 'utf8');
      // Rewrite relative url() statements into absolute statements so the
      // resulting combined CSS file will still work.
      //contents = rewriteRelativeURLs(contents, rootPath, outputFile);
      chunks.push(contents);
    });
    fs.writeFileSync(outputFile, chunks.join('\n'), 'utf8');
    minify(outputFile, 'yui-css');
  }
  exports.combineCSS = combineCSS;

  // Combine one or more JavaScript files into a single compressed file.
