...
var UGLIFY = 'node_modules/node-minify/node_modules/uglify-js/bin/uglifyjs';
...
YUI().use(['yui'], function(Y) {
  ...
  function combineJs(files, outputFile) {
    var sourceMapName = syspath.basename(outputFile, '.js') + '-source-map';
    var options = ['--source-map', sourceMapName, '--source-map-root=source',
      '--output', outputFile];
    ...
    Y.Array.each(files, function(inputFile) {
      options.push(syspath.relative(process.cwd(), inputFile));
    });
    var uglify = child_process.spawn(UGLIFY, options, {stdio: 'inherit'});
    uglify.on('exit', function(code) {
      if (code !== 0) {
        throw 'error running uglify';
      }
    });
  }
  ...
  function getYUIFiles(reqs) {
    ...
    loader = new Y.Loader({
      ...
      filter: 'RAW', // We do not want minified source.
      require: reqs});
