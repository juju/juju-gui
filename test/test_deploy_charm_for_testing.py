...
class TestWaitingForService(unittest.TestCase):
    ...
    def test_service_started(self):
        ...
        def get_service_state():
            return 'started'
        # The lack of assertion is because the sleep function would have been
        # called if wait_for_service did not return immediately, but the
        # passed sleep function is None, so the fact that no exception is
        # raised shows that no sleeping was done.
        wait_for_service(get_service_state, sleep=None)
