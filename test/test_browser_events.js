  ...
  describe('browser event handlers', function() {
    ...
    describe('highlight events', function() {
      ...
      beforeEach(function() {
        ...
        browser.set('db', db);
        utils.makeStubMethod(db, 'findUnrelatedServices', [
          db.services.getById('haproxy')
        ]);
      });

      it('sets highlight flag on selected service', function() {
        var mysql = db.services.getById('mysql');
        assert.equal(mysql.get('highlight'), undefined,
      ...
      it('sets unrelated services\' fade flag on highlight', function() {
        ...
        assert.equal(mysql.get('fade'), undefined,
                     'target service should not have flag set initially');
        assert.equal(wordpress.get('fade'), undefined,
                     'related service should not have flag set initially');
        assert.equal(haproxy.get('fade'), undefined,
    ...
    describe('hide/fade events', function() {
      ...
      beforeEach(function() {
        ...
        db.services.add([
          {id: 'mysql', units: units},
          {id: 'wordpress', units: units}
        ]);
        db.addUnits(units.toArray());
        browser.set('db', db);
      });
